...
public class DBHelper {
     ...
     * 数据库名称
     */
    private final String DB_NAME = "com-r-sun-cn.db";
    ...
    private DBHelper() {
        if (SunHttp.Configure.get().getContext() == null) {
            throw new NullPointerException("RHttp not init!");
        }
        context = SunHttp.Configure.get().getContext();
        initDB(context);
    }
    ...
    private void initDB(Context context) {
        if (db == null) {
            ...
            config.debugged = SunHttp.Configure.get().isShowLog(); // open the log
